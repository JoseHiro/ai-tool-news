...
```
【Grok リサーチ結果】（A〜F を貼る。なければ削除してよい）
---
▼ テンプレートA（AI ツールニュース）
（ここに貼る）
...
▼ テンプレートE（シンプル・バイラル系アプリ）
（ここに貼る）

▼ テンプレートF（App Store 急上昇 & カテゴリ参入余地）← 毎日必須
（ここに貼る）
...
アイデアは Template F の急上昇・隙間ポジションを最初の起点にすること。
...
| F | App Store 急上昇 & カテゴリ参入余地チェック | 毎日 |
| C | ニッチコミュニティ・海外ヒット・ユーザー不満の深掘り | 任意 |
| D | 個別アイデアの App Store 被り確認 | アイデア確定前に都度 |

**テンプレートFの使い方:** 毎日実行。深掘りカテゴリは下記ローテーションに従う。急上昇アプリと隙間ポジションを必ず確認し、アイデア生成の最初の起点にする。

**テンプレートCの使い方:** 「特定ニッチを深掘りしたい」「ユーザー不満をリアルタイムで拾いたい」日だけ使う。F で見えてきたカテゴリを深掘りする用途が主。
...
#### 📊 テンプレートF — App Store 急上昇 & カテゴリ参入余地チェック（毎日必須）

※ **テンプレートA〜E とは完全に独立した検索です。**
...
```
以下の3セクションで App Store のランキングを調査してください。前の検索とは無関係の独立したリサーチです。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【セクション1: 今週の急上昇・新規参入アプリ】

App Store（日本・米国）の「急上昇」「Breaking Out」「新着注目」に今週入ってきたアプリを10件以上。

各アプリ:
- アプリ名 / カテゴリ / 価格 / 評価数 / 開発者（個人 or 企業）
- 急上昇の理由（SNS バズ / プレスリリース / 季節需要 など、わかる範囲で）
- 個人開発者のアプリか否か

特に注目: 個人開発のアプリ / 機能がシンプルなのに上位に入ったもの / 価格が安いのに支持されているもの

━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【セクション2: 今日の深掘りカテゴリ（曜日ローテ）】

対象カテゴリ: [曜日に対応するカテゴリを入れる]

App Store（日本・米国）のこのカテゴリを調査:

1. トップ有料アプリ 上位10件
   アプリ名 / 価格 / 評価数 / 最終アップデート年月 / 個人 or 企業

2. 個人開発者が入れそうな隙間
   - 上位に 2年以上更新されていないアプリがあるか
   - 日本語版がない米国上位アプリはあるか
   - 評価数が少ない（500未満）のに上位に残っているアプリはあるか（競合が弱い証拠）

3. 価格帯の空白
   - ¥250〜¥600 の買い切り価格帯に何件あるか
   - サブスクが多いカテゴリで買い切りを出せる余地はあるか

━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【セクション3: 日米ギャップ サマリー】

セクション1・2 を踏まえて:
- 米国で人気だが日本版がないアプリ（上位3件）
- 日本でランクインしているが海外未展開のアプリ（上位3件）
- 個人開発者が今すぐ参入するなら、このカテゴリのどのポジションが最もチャンスがあるか（1〜2文で）
...
1. **Template F の急上昇・隙間ポジション** — 実際にランキングで動いているデータを最初に見る。「今週上位に入ってきた個人開発アプリ」と「2年以上更新されていないのに上位にいるアプリ」が最も信頼できる参入余地のシグナル
2. **Template F の日米ギャップ** — 米国上位・日本語版なし or 日本発・海外未展開のアプリを起点にする
3. **Template B・E の成功事例とバズアプリ** — Grok が拾ってきた「今ホット」な実績・拡散事例をそのまま使う
4. 今日の AI ツールニュース（A 結果）と組み合わせられる個人開発アイデア
5. 日本固有の文化・制度・慣習に根ざした問題（他国では問題にならない）

**❌ やらないこと:** Template F を見る前にニッチを決める。ランキングデータを先に確認してから「面白い」と感じたものを起点にする。
